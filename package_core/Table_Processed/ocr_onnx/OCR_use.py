...
class det_rec_functions(object):
    ...
    def get_boxes(self, name, show, image1):
        ...
        for det_list in det_lists:
            ...
            x2 = det_list[2][0] * 3
            y2 = det_list[2][1] * 3
            yolo_cor = [x1,y1,x2,y2]
            Yolo_match_list.append(yolo_cor)
            # draw.rectangle([x1, y1, x2, y2], outline=(0, 0, 255))
        # if not os.path.exists('det_sign'):
        #         os.makedirs('det_sign')
        # img.save(f'det_sign/{name}.png')
        return det_boxs,Yolo_match_list
    ...
    def get_rotate_crop_image(self, img, points):
        ...
        if dst_img_height * 1.0 / dst_img_width >= 1.2:  # 原为1.5
...
def ONNX_Use(image, name):
    ...
    dt_boxes,Yolo_data= ocr_sys.get_boxes(name, show=False, image1=image) # Yolo_data为修改后的坐标数据，该坐标数据=原图上的检测框坐标，该变量格式为[[x1,y1,x2,y2],[x1,y1,x2,y2],....]
    # det_boxes准换为列表形式：
    TextCoordinates = []
    for det_list in dt_boxes:
        ...
        for dets in det_list:
            ...
            x.append(int(arr_to_list[0]))
            y.append(int(arr_to_list[1]))
        TextCoordinates.append([round(min(x)/2+max(x)/2),round(min(y)/2+max(y)/2)])
    ...
    return text_list, TextCoordinates


if __name__ == '__main__':
    image_path = 'Ocr/Image/9.png'
    name = image_path.split('/')[1]
    name = name.split('.')[0]
    image = cv2.imread(image_path)
    text_list, TextCoordinates = ONNX_Use(image, '')
    for index in range(text_list.__len__()):
        print(text_list[index],TextCoordinates[index])
