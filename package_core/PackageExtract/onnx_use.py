...
parent_dir = str(os.path.dirname(current_dir)) # 上一级目录路径
root_dir = str(os.path.dirname(parent_dir))  # 再上一级目录路径
    ...
    def recognition_img(self, dt_boxes,Is_crop):
        ...
        for box in dt_boxes:
            ...
            if Is_crop:
                ...
                crop.save(f'det_crop/crop_{i}.jpg')
                i += 1
...
def ONNX_Use(image,image_path, dt_boxes):
    ...
    results, results_info = ocr_sys.recognition_img(dt_boxes, Is_crop=False)
    text_list = []
...
def Run_onnx(image_path, dt_boxes):
    ...
    # ONNX运行代码
    text_list = ONNX_Use(image, image_path, dt_boxes)
...
def Run_onnx_det(image_path):
    ...
    # ONNX运行代码
    text_list = ONNX_Det(image, image_path)
    new_text = np.zeros((0, 4))
    mid = np.zeros((4))
    for i in range(len(text_list)):
        mid[0] = text_list[i][0][0]
        mid[1] = text_list[i][0][1]
        mid[2] = text_list[i][1][0]
        mid[3] = text_list[i][2][1]
        new_text = np.r_[new_text, [mid]]
    return new_text
if __name__=='__main__':
    print("当前目录路径:", current_dir)
    print("上一级目录路径:", parent_dir)
    print("再上一级目录路径:", root_dir)
    print(os.path.exists('C:/Users/14685/PycharmProjects/pythonProject/packagefiles/PackageExtract/Result/Package_extract/data/top.jpg'))
...
# paddle2onnx --model_dir onnx_study/det --model_filename inference.pdmodel --params_filename inference.pdiparams --save_file onnx_study/onnx_det/det_model.onnx
