...
def make_parser():
    ...
    parser.add_argument(
        ...
        "--model",
        type=str,
        default="yolox_onnx/BGA_body.onnx",
    ...
    parser.add_argument(
        ...
        "--output_dir",
        type=str,
        default='onnx_output/BGA_body.onnx',
...
def find_the_only_body(img_path):
    ...
    if len(location) > 1:
        ...
        while len(new_location) > 1:
            ...
            if len(new_new_location) == 0:
                new_new_location = location
    ...
    if len(location) == 1:
        ...
        # cv2.waitKey(0)
    return YOLOX_body
def begain_output_top_body_location():
    """封装入口：输出顶视本体检测结果。"""
    global YOLOX_body
    global location
    onnx_inference()
