...
def preprocess(img: np.ndarray, input_size: Sequence[int], swap: Tuple[int, int, int] = (2, 0, 1)):
    """按比例缩放并填充图像，返回模型输入以及缩放比例。"""
...
def nms(boxes: np.ndarray, scores: np.ndarray, nms_thr: float) -> Iterable[int]:
    """执行单类别 NMS，返回保留框的索引。"""
...
def demo_postprocess(outputs: np.ndarray, img_size: Sequence[int], p6: bool = False) -> np.ndarray:
    """将模型输出映射回输入图片尺寸。"""
...
def vis(
    ...
    boxes: np.ndarray,
    scores: np.ndarray,
    cls_ids: Iterable[int],
    conf: float = 0.5,
    class_names: Optional[Sequence[str]] = None,
) -> np.ndarray:
    """在图像上绘制检测框与标签，返回绘制后的图像。"""
