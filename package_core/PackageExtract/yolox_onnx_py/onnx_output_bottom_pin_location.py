...
def make_parser():
    ...
    parser.add_argument(
        ...
        "--model",
        type=str,
        default="yolox_onnx/BGA_bottom_pin.onnx",
    ...
    parser.add_argument(
        ...
        "--output_dir",
        type=str,
        default='onnx_output/BGA_bottom_pin.onnx',
        ...
        while len(new_location) > 1:
            ...
            if len(new_new_location) > 1:
                for i in range(len(new_new_location)):
                    mark_location[i] = sqrt((new_new_location[i][2] - new_new_location[i][0]) ** 2 + (new_new_location[i][3] - new_new_location[i][1]) ** 2)
...
def onnx_inference():
    ...
    output_pairs_data_location(np.array(final_cls_inds), np.array(final_boxes))  # 将yolox检测的pairs和data进行匹配输入到txt文本中
    '''
...
def begain_output_bottom_pin_location():
    """封装入口：输出底视引脚检测结果。"""
    global YOLOX_body
    global location

    onnx_inference()
    img_path = 'bga_bottom/bottom.jpg'
    YOLOX_body = find_the_only_body(img_path)
    return YOLOX_body

if __name__ == '__main__':
    YOLOX_body = begain_output_bottom_pin_location()
    print(YOLOX_body)
